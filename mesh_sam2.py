...
def get_plate_group_day_well(file_name):
    ...
    # Assume the filename format is "P1Aw1D10-00ampsfilt.jpg"
    ...
    day = base_name[6] if len(base_name) >= 4 else None      # 6th character for Day
    well = base_name[4] if len(base_name) >= 6 else None      # 4th characters for Well
...
# Extract unique groups from files
def extract_groups_from_files(files):
    groups = set()
    for file in files:
        _, group, _, _ = get_plate_group_day_well(file)
        if group:
            groups.add(group)
    return list(groups)
...
def extract_cycle_and_frame(filename):
    """
    Extract the cycle (10, 20, 30) after 'x' and numeric frame value after the last hyphen.
    ...
    frame_match = re.search(r"-(\d+)", filename)  # Extracts last numeric part after '-'
...
def extract_numeric_part2(filename):
    """Extract numeric part from filename (e.g., '100000.jpg' -> 100000)."""
    try:
        # Extract the numeric part before the file extension 
        numeric_part = int(filename.split('.')[0])  # This should give you 100000, 100003, etc.
        return numeric_part
    except (IndexError, ValueError) as e:
        print(f"Warning: Could not extract numeric part from filename: {filename}. Error: {e}")
        return None  # Return None if extraction fails

def group_files_by_main_prefix(files):
    """
    Group files by main prefix up to 'x' (e.g., "P1Aw1D0").
    """
    grouped_files = {}
    for file in files:
        main_prefix = file.split('x')[0]  # Prefix up to 'x', e.g., "P1Aw1D0"
...
# Process the TIFF files, convert them to JPEG, and organize them by group
def process_images():
    ...
    for prefix, group_files in grouped_files.items():
        ...
        for idx, file_name in enumerate(sorted_group_files):
            ...
            try:
                ...
                if file_path.lower().endswith('.tiff') or file_path.lower().endswith('.tif'):
                    ...
                    jpeg_filename = f"{group_number:01d}{cycle:02d}{frame:02d}.jpg"
        ...
        process_jpeg_group_with_sam2(group_number, group_dir)   #### coding out to run groups faster for fixing strains
...
clicked_points = []
clicked_labels = []
# Initialize a variable to store Object 3's contour from the first frame
stored_contour_obj_3 = None
...
def visualize_predictions(prompts, group_dir, frame_names, predictor, inference_state, ann_frame_idx, group_number):
    ...
    global stored_contour_obj_3, consolidated_data_list, dot_y_coord_bottom, dot_y_coord_bottom2, axs, fig, cycle, avg_y
    # Initialize the distances list for exporting lengths and widths
    # Create a figure with three subplots
    distances_list = []
    consolidated_data = []
    min_width_length = None  # Initialize to a default value
    # Get the specific JPEG filename for this frame
    jpeg_filename = frame_names[ann_frame_idx]
    jpeg_file_path = os.path.join(group_dir, jpeg_filename)
    # Get the TIFF file name from the unique mapping using jpeg_filename
    tiff_file_name = jpeg_to_tiff_mapping.get(jpeg_filename, None)
    # Load the original image
    img = Image.open(jpeg_file_path)
    img_np = np.array(img)  # Convert PIL image to numpy array for OpenCV
    width, height = img.size
    
    # Processed image for visualization in the second subplot
    ...
    # Draw the largest contours on the second subplot
    for obj_id, contour in largest_contours.items():
        if contour is not None:  # Check if a contour was found
            if obj_id == 1:
                color = (0, 255, 0)  # Green for ID 1
                cv2.drawContours(img_np, [contour], -1, color, thickness=2)  # Outline the largest contour
            if obj_id == 2:
                color = (255, 0, 0)  # Blue for ID 1
                cv2.drawContours(img_np, [contour], -1, color, thickness=2)  # Outline the largest contour
    
    # Clear the axes from any previous content
    axs = plt.gca()  # Get current axis for drawin
    axs.clear()
    axs.imshow(img_np[::-1])
    axs.axis('off')
    axs.invert_yaxis()  # Flip the y-axis of ax[2]
    legend_entries = []
  
    ### Third Subplot Logic for Object 1 (EHT)
    if largest_contours[1] is not None:  # Check if we found a contour for EHT
        # Get object 1's contour and calculate mid-way x-coordinate
        contour_obj_1 = np.squeeze(largest_contours[1])
        if contour_obj_1.ndim == 1:
            contour_obj_1 = np.reshape(contour_obj_1, (-1, 2))
        x_coords_obj_1 = contour_obj_1[:, 0]
        mid_x_obj_1 = (np.min(x_coords_obj_1) + np.max(x_coords_obj_1)) // 2  # Mid-way x-coordinate

        # Check for Object 2
        if largest_contours[2] is not None:  # Object 2 exists
            contour_obj_2 = np.squeeze(largest_contours[2])
            contour_area_2 = cv2.contourArea(largest_contours[2])
            if contour_obj_2.ndim == 1:
                contour_obj_2 = np.reshape(contour_obj_2, (-1, 2))
            # Compute centroid using moments
            M = cv2.moments(largest_contours[2])
            if M["m00"] != 0:  # Avoid division by zero
                centroid_x_obj_2 = int(M["m10"] / M["m00"])
                centroid_y_obj_2 = int(M["m01"] / M["m00"])
            else:
                centroid_x_obj_2, centroid_y_obj_2 = 0, 0
            # Flip y-coordinate for the flipped y-axis
            flipped_centroid_y_obj_2 = height - centroid_y_obj_2
        else:
            centroid_x_obj_2, flipped_centroid_y_obj_2 = 0, 0
            print("Object 2 not found.")

        # If both Object 2 and Object 3 exist, calcualte length - certian pixels from centroid for pillar tip.. 
        if largest_contours[2] is not None:
            # pillar_y_from_magnet = flipped_centroid_y_obj_2 - 300 #### FOR THE THICKNESS TESTING CODE
            pillar_y_from_magnet = flipped_centroid_y_obj_2 - 275 #### FOR THE BIG MAG CODE
            
            # Calculate the length of the line
            use_y = height - avg_y
            line_length = np.abs(pillar_y_from_magnet - use_y)
        else:
            line_length = 0
        
        # Plotting lines
        axs.plot(centroid_x_obj_2, use_y, 'g*', markersize = 12) # plotting pillar tip line 
        axs.plot(centroid_x_obj_2, pillar_y_from_magnet, 'g*', markersize = 12) # plotting pillar tip line 
        ...
        # New section to find widths across Object 1 (EHT)
        if largest_contours[1] is not None:  # Ensure EHT contour exists
           
            y_threshold = 35  # Define a threshold for how close y-coordinates should be to the target y-coordinates
            y_coords_near_max = []
            y_coords_near_min = []
            # Initialize variables to track maximum widths for top and bottom
            max_width_top = 0
            max_width_bottom = 0
            ...
            # Find y-coordinates near piston and grip max/min points
            for y in contour_obj_1[:, 1]:
                y_inverted = max_y - y
                # print(y-pillar_y_from_magnet)
                if abs(y_inverted - pillar_y_from_magnet) < y_threshold:
                    y_coords_near_max.append(y)
                if abs(y_inverted - use_y) < y_threshold:
            ...
            # Calculate widths and track maximum widths for top and bottom
            for y_coord in combined_y_coords:
                ...
                if len(x_coords_at_y) > 0:  # If there are points at this y-coordinate
                    ...
                    # Track the maximum width for top and bottom regions
                    if y_coord in y_coords_near_max:  # If it's near the top
                    ...
                    elif y_coord in y_coords_near_min:  # If it's near the bottom
        ...
        if max_width_y_coord_top is not None:
            axs.plot([min_x_at_max_width_top, max_x_at_max_width_top],
                        [max_y - max_width_y_coord_top, max_y - max_width_y_coord_top], 
                        'g--', linewidth=2, label=f'Max Width Top: {max_width_top}')  # cyan line

        if max_width_y_coord_bottom is not None:
            axs.plot([min_x_at_max_width_bottom, max_x_at_max_width_bottom],
                        ...
                        'g--', linewidth=2, label=f'Max Width Bottom: {max_width_bottom}')  # magenta line
        ...
        #### GETTING EHT WIDTHS / LENGTHS 
        contour_frame = np.array(img.convert('RGB'), dtype=np.uint8)
        # Convert the contour to numpy array for easier manipulation
        contour = np.squeeze(largest_contours[1])
        if contour.ndim == 1:
        ...
        interp_cumulative_distances = np.linspace(0, cumulative_distances[-1], 2000)   # could increadse value if want more points... also all images are OG contour, not smoothed
        ...
        #### Finding EHT Length %s that Align with the Width measurements for grip / piston

        # Find the corresponding percentages for max_width_y_coord_top and max_width_y_coord_bottom
        if max_width_y_coord_top is not None and max_width_y_coord_bottom is not None:
            # Calculate percentage for the maximum width coordinates
            percentage_top = np.clip(((max_y - max_width_y_coord_top) / max_tissue_length) * 100, 0, 100)
            percentage_bottom = np.clip(((max_y - max_width_y_coord_bottom) / max_tissue_length) * 100, 0, 100)
            # Find closest percentage in the defined percentages array
            closest_percentage_top = percentages[np.argmin(np.abs(percentages - percentage_top))]
        ...
        amps = int(jpeg_id[-2:])  # Last two digits as amplitude
        cycle = int(jpeg_id[-4])  # 4th to last digit
        print(cycle)

        # Initialize variables to store minimum width and corresponding percentage
        ...
        for percentage in percentages:
            ...
            width_at_percentage = max_x_at_y - min_x_at_y
            length_at_percentage = max_y_at_x - min_y_at_x
            shift = height - max_OG_y
            # Append data to the distances list
            ...
            ### MID WIDTH
            # Check if percentage is within 35% to 65%
            if 35 <= percentage <= 65:
                if 25 < width_at_percentage < min_width_within_range:
                    min_width_within_range = width_at_percentage
        ...
        if min_width_length is None:
            ...
            # Plot the minimum width line on axs[1] or axs[2] at the corresponding percentage (min_width_length)
            axs.plot([min_x_at_y_min, max_x_at_y_min], [min_width_length + shift, min_width_length + shift], 'g--', linewidth=2)




        # Append a text-only entry for the legend
        legend_entries.append(f"Mid Width: {min_width_within_range:.2f}")  # Adjust units as needed

        # Create the legend and store it in a variable
        legend = axs.legend(legend_entries, loc='upper left', bbox_to_anchor=(1, 0.25), borderaxespad=0.)

        # Hide the line symbols in the legend
        ...
        excel_file_path = os.path.join(group_dir, 'data.xlsx')
        # Append the DataFrame to the Excel file
        if os.path.exists(excel_file_path):
            # Load existing data to find the starting row for appending
            with pd.ExcelFile(excel_file_path) as xls:
                # Read the existing data to get the current number of rows
                existing_df = pd.read_excel(xls, sheet_name='data')
                startrow = existing_df.shape[0]  # Start appending after the last row
            # Write the new data without headers
            with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                distances_df.to_excel(writer, index=False, header=False, startrow=startrow, sheet_name='data')
        else:
            # Write the DataFrame to the Excel file with headers
        ...
        consolidated_data.append(pd.DataFrame({
            ...
            'P. Grip Width': [max_width_top],
            'S. Grip Width': [max_width_bottom],
            'EHT Length': [line_length],
        }))

    # After the loop, convert the consolidated data list to a DataFrame
    if consolidated_data:  # Check if consolidated_data is not empty
        consolidated_df = pd.concat(consolidated_data, ignore_index=True)
        # Remove duplicate rows in the consolidated_df
        consolidated_df = consolidated_df.drop_duplicates()
    else:
        print("No consolidated data to save.")
        return  # Exit the function if there's no data to save
        
    # # Ensure `consolidated_df` is defined
    # if 'consolidated_df' not in locals() or consolidated_df is None:
    #     print("Warning: `consolidated_df` is not defined. Initializing an empty DataFrame.")
    #     consolidated_df = pd.DataFrame()  # Initialize to an empty DataFrame as fallback

    # Define the path for the consolidated Excel file
    consolidated_excel_file_path = os.path.join(group_dir, 'data_consolidated.xlsx')
    # Check if the consolidated Excel file exists
    if os.path.exists(consolidated_excel_file_path):
        # Load existing data to find the starting row for appending
        with pd.ExcelFile(consolidated_excel_file_path) as xls:
            existing_consolidated_df = pd.read_excel(xls, sheet_name='data_consolidated')
            # Combine existing DataFrame with the new one and drop duplicates
            combined_df = pd.concat([existing_consolidated_df, consolidated_df]).drop_duplicates().reset_index(drop=True)
            startrow_consolidated = existing_consolidated_df.shape[0]  # Start appending after the last row

        # Append the unique DataFrame to the existing Excel file without headers
        with pd.ExcelWriter(consolidated_excel_file_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
            combined_df.to_excel(writer, index=False, header=False, startrow=startrow_consolidated, sheet_name='data_consolidated')
    else:
        # Write the DataFrame to the consolidated Excel file with headers
        with pd.ExcelWriter(consolidated_excel_file_path, engine='openpyxl', mode='w') as writer:
    ...
    plt.tight_layout()
    # Save the figure before drawing and closing
    output_image_path = os.path.join(group_dir, f"{tiff_file_name}_{cycle}_{amps}.png")
...
def process_jpeg_group_with_sam2(group_number, group_dir):
    ...
    global clicked_points, clicked_labels, current_object, clicks_per_object_eht_positive, clicks_per_object_eht_negative, clicks_per_object_piston_positive, clicks_per_object_piston_negative, clicks_per_object_grip_positive, clicks_per_object_grip_negative, clicks_per_object_magnet_positive, clicks_per_object_magnet_negative,  jpeg_to_tiff_mapping, axs, cid, frame_0_top_percentage, frame_0_bottom_percentage, dot_y_coord_bottom, dot_y_coord_bottom2, avg_y
    ...
    sam2_checkpoint = r"C:\Users\scoeyman\Desktop\SAM MODEL\sam2_hiera_large.pt"
    model_cfg = "sam2_hiera_l.yaml"
    predictor = build_sam2_video_predictor(model_cfg, sam2_checkpoint, device=device)
    # Initialize the inference state using the JPEG frames directory
    inference_state = predictor.init_state(video_path=group_dir)
    prompts = {}  # Hold all the clicks we add for visualization
    ann_frame_idx = 0 # FRAME TO PROCESS
    # List all files in the JPEG directory and print for debugging
    all_files = os.listdir(group_dir)
    # Filter and sort JPEG files numerically
    frame_names = [f for f in all_files if f.endswith('.jpg')]
    ...
    if eht_loaded_points:  # If clicks exist, use them
        ...
        # Clear the current axis and show the image for EHT
        ...
        plt.close()
        # After collecting clicks for EHT, process the clicks
        if clicked_points:
            eht_points = np.array(clicked_points, dtype=np.float32)
            eht_labels = np.array(clicked_labels, dtype=np.int32)
            prompts[ann_obj_id] = eht_points, eht_labels  # Store points and labels in prompts
            save_clicks_to_file(eht_points, eht_labels, eht_clicks_file_path)
    ...
    if grip_loaded_points:  # If clicks exist, use them
        ...
        avg_y = np.mean([point[1] for point in grip_points])
        avg_x = np.mean([point[0] for point in grip_points])  # Optional if you want the center average
...
# Delete output files in the folder
